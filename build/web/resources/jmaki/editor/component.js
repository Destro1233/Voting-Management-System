/**
 *  jMaki Editor Widget using  NicEdit
 * 
 *  For more on NiceEdit visit http://nicedit.com/
 * 
 */            
jmaki.namespace("jmaki.widgets.jmaki.editor");

jmaki.widgets.jmaki.editor.Widget = function(wargs) {

   var publish = "/jmaki/editor";
   var _widget = this;
   _widget.container = document.getElementById(wargs.uuid);

    this.resize = function() {
        _widget.dim = jmaki.getDimensions(_widget.container);         	
        var neps = jmaki.getElementsByStyle('nicEdit-panel', _widget.container);
        if (neps && neps.length >0) {
     	    var h= neps[0].clientHeight;
     	    if (/MSIE 6/i.test(navigator.userAgent)) h+= 68;
            var mains = jmaki.getElementsByStyle('nicEdit-main', _widget.container);
            mains[0].parentNode.style.width = (_widget.dim.w - 2) + "px";
            mains[0].parentNode.style.height = (_widget.dim.h - h - 25) + "px";          
            mains[0].style.overflowY = "auto";
            mains[0].style.height = (_widget.dim.h - h - 25) + "px";
            mains[0].style.width = (_widget.dim.w - 2) + "px";            
        } else {
        	_widget.contentPane.style.overflowY = "auto";
        	_widget.contentPane.style.height = _widget.dim.h - 25 + "px";
        }
    };
    
    function stopEdit() {
    	_widget.wrapper.removeInstance(wargs.uuid + "_scroller");
        _widget.editMode = false;
        jmaki.publish(publish + "/onSave", _widget.getValue());
    }
    
    function startEdit() {
    	_widget.wrapper.panelInstance(wargs.uuid + "_scroller",{hasPanel : true});
    	_widget.editMode = true;
    	jmaki.publish(publish + "/onSave", _widget.getValue());
    }
    
    function enableOptions() {
    	if (!_widget.options) {
    	    _widget.options = document.createElement("div");
    	    _widget.options.style.fontSize = "0.7em";
            _widget.options.style.textDecoration = "underline";
    	    if (jmaki.MSIE) _widget.options.style.styleFloat = "right";
			else _widget.options.style.cssFloat = "right";
    	    _widget.options.style.paddingTop = "3px";
			_widget.options.style.paddingRight = "3px";
    	    _widget.options.style.cursor = "pointer";
    	    _widget.container.appendChild(_widget.options);
    	    _widget.options.onclick = function() {
    	    	if (_widget.editMode) {
    	    		stopEdit();
    	    		enableOptions();
    	    		_widget.resize();
    	...
    	if (_widget.editMode) {
    	    _widget.options.innerHTML = "[Edit Complete]";
    	} else {
    		_widget.options.innerHTML = "[Edit]";
    	}
    }
    
    this.setValue = function(content) {
    	if (_widget.editMode) {	
    	    _widget.wrapper.fireEvent("save");
            var i=  nicEditors.findEditor( wargs.uuid + "_scroller").nicInstances[0];
            if (i.frameContent) i.frameContent.innerHTML = content;
            if (i.elm) i.elm.innerHTML = content;
    	} else {
    	    _widget.contentPane.innerHTML = content;
    	}
    };    
    ...
    this.getValue = function() {
    	...
    	if (_widget.editMode) {	
    	    _widget.wrapper.fireEvent("save");
            var i=  nicEditors.findEditor( wargs.uuid + "_scroller").nicInstances[0];
            if (i.frameContent) content = i.frameContent.innerHTML;
            if (i.elm) content = i.elm.innerHTML;
    	} else {
    	    content = _widget.contentPane.innerHTML;
    	}
    ...
    this.postLoad = function() {
      ...
      _widget.contentPane = document.getElementById(wargs.uuid + "_scroller");    
      if (wargs.value) _widget.contentPane.innerHTML = wargs.value;

      _widget.wrapper = new nicEditor({ iconsPath : wargs.widgetDir + '/images/nicEditorIcons.gif',fullPanel : true});
      _widget.resize();
      enableOptions(); 
    };
